...
app.post('/api/register', (req, res) => {
  ...
  bcrypt.hash(password, 10, (err, hash) => {
    ...
    db.run(`INSERT INTO interviewers (name, email, password, job_family, level, is_manager, is_bar_raiser) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [name, email, hash, job_family, level, is_manager ? 1 : 0, is_bar_raiser ? 1 : 0],
      function(err) {
        if (err) {
  ...
  db.get('SELECT * FROM interviewers WHERE email = ?', [email], (err, user) => {
    ...
    bcrypt.compare(password, user.password, (err, match) => {
      ...
      const token = jwt.sign(
        { id: user.id, email: user.email, job_family: user.job_family, level: user.level, is_manager: user.is_manager, is_bar_raiser: user.is_bar_raiser },
        JWT_SECRET,
        { expiresIn: '24h' }
      );
      
      res.json({ token, user: { id: user.id, name: user.name, email: user.email, job_family: user.job_family, level: user.level, is_manager: user.is_manager, is_bar_raiser: user.is_bar_raiser } });
    });
  });
});
